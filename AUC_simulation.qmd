...
We utilized the simulation approach to serve the three aims We did simulation in the context of pain research with the outcome evaluated using the Numeric Rating Scale (NRS), with the score ranging from 0 (no pain) to 10 (most intensive).

## Aim 1: Comparing power {#sec-aim1}

In this section, we first simulate the the responses in the NRS scale in a parallel-group RCT with two arms: new drug (*"Treatment"*) compare to an active control (*"Control"*) , over the period of 24 hours. We assumed patients response to both treatments with *"typical curve"* with 3 phases: fast response, maintainance, and losing effect.

We then compared the power of non-parametric GROOT estimate with traditional longitudinal approachs of analyzing categorical reponse. The statistical power was calculated as the proportion of time reaching statistical significance in a 1000-iteration run. For simplicity, we choose the Clinicially Meaningful Response (CMR) $\delta$ as $>50 \%$ improvement in the NRS using a modified percent change from baseline.
...
**Create response dataset (change from baseline i.e., ≥50% improvement)**
...
The non-parametric GROOT provide an accurate way to measure area under the response curve. However, it has some disadvantages:
...
To address that problem, we proposed another approach using model with flexible terms on time to model the response curve of patients and calculate the GROOT based on that model.

### Methods & Results

We propose a simple method for model-based GROOT by two main step:
...
In this section, we illustrated the process of applying three well-known methods to estimate causal effects in the context of existing confounders: parametric g-formula, inverse probability weighting, and double-robust estimator. We did a simulation to compare the performance of these methods under different scenarios.
...
-   Data-generating mechanism: Four simulation mechanisms were set: small confounding, medium confounding, large confounding, and large confounding with model misspecification. Details are presented in section XXX.
...
-   Methods: The the g-methods were used: Parametric g-formula (PG), Inverse Probability Weighting (IPW), and Double-robust estimator (DRE). The estimates were compared to the unadjusted estimates and the true values.
...
We first set up three confounding mechanisms (i.e., *"small"*, *"medium"*, and *"large"*) illustrating different situations of *"small"*, *"medium"*, and *"large"* confounding effects in terms of the magnitude of absolute bias for GROOT difference as well as the number of confounders (i.e., one, two, and three, respectively). Figure XXX shows the Directed Acyclic Graphs (DAG) of these three confounding mechanisms. To illustrate a problem that usually happens in real-world situations, which is unobserved confounders, we set U as an unobserved variable in the DAG.
...
-   **Model misspecification**: In this scenario, we used the same confounding mechanism as scenario 3, except that we intentionally misspecified the models. In particular, in the model for treatment assignment (i.e., for IPW), we included only intercept instead of $X_1$, $X_2$, and $X_3$. Similarly, for the outcome regression model (i.e., g-formula), the model with only intercept was used. Thus, creating three situations for DRE:

    -   The model for treatment assignment was misspecified but the outcome regression model was correct (DRE-A)

    -   The model for treatment assignment was correct but the outcome regression model was misspecified (DRE-Y).

    -   Both treatmet and outcome models were misspecified (DRE-AY)

#### Data generation

The data for each scenario was generated according to its confounding mechanism. The magnitude of the relationship between variables was reflected by the coefficients. For each scenario, we created datasets with 1000 samples. A loop of 1000 iterations was then performed, and the everage bias over 1000 iterations were obtained. The R code is provided in the [this GitHub repository](https://github.com/khuongquynhlong). The details of each scenario are given in the following sections.
...
&Y_t^{(1)} \sim \text{Bin}(n, p_y) ;~~~~~~~~~~ \text{with: logit}(p_{y^{(1)}}) = \text{logit}(f_0(t)) + I_{(\text{A}=1)} + {\color{Red}0.5^{*} \text{U}} \nonumber\\
&Y_t^{(0)} \sim \text{Bin}(n, p_y) ;~~~~~~~~~~ \text{with: logit}(p_{y^{(0)}}) = \text{logit}(f_0(t)) + I_{(\text{A}=1)} + {\color{Red}0.5^{*} \text{U}} \nonumber\\
...
&X_3  \sim N(0, 2) \nonumber\\
&A \sim \text{Bin}(n, p_a) ;~~~~~~~~~~~~~~ \text{with: logit}(p_a) = 0.5^{*}X_1 + 0.5^{*} X_3 \nonumber\\
&Y_t^{(1)} \sim \text{Bin}(n, p_y) ;~~~~~~~~~~ \text{with: logit}(p_{y^{(1)}}) = \text{logit}(f_0(t)) + I_{(\text{A}=1)} + {\color{Red}0.5^{*} \text{U} + 0.5^{*} X_3} \nonumber\\
&Y_t^{(0)} \sim \text{Bin}(n, p_y) ;~~~~~~~~~~ \text{with: logit}(p_{y^{(0)}}) = \text{logit}(f_0(t)) + I_{(\text{A}=1)} + {\color{Red}0.5^{*} \text{U} + 0.5^{*} X_3} \nonumber\\
&Y_t =  Y_t^{(1)*}A + Y_t^{(0)*}(1 - A) \nonumber\\ 
\end{align*}
```
...
&X_3  \sim N(0, 2) \nonumber\\
&A \sim \text{Bin}(n, p_a) ;~~~~~~~~~~~~~~ \text{with: logit}(p_a) = 0.5^{*}X_1 + 0.5^{*} X_3 \nonumber\\
&Y_t^{(1)} \sim \text{Bin}(n, p_y) ;~~~~~~~~~~ \text{with: logit}(p_{y^{(1)}}) = \text{logit}(f_0(t)) + I_{(\text{A}=1)} + {\color{Red}0.5^{*} \text{U} + 0.75^{*} X_3 + 0.75^{*} X_2} \nonumber\\
&Y_t^{(0)} \sim \text{Bin}(n, p_y) ;~~~~~~~~~~ \text{with: logit}(p_{y^{(0)}}) = \text{logit}(f_0(t)) + I_{(\text{A}=1)} + {\color{Red}0.5^{*} \text{U} + 0.75^{*} X_3 + 0.75^{*} X_2} \nonumber\\
&Y_t =  Y_t^{(1)*}A + Y_t^{(0)*}(1 - A) \nonumber\\ 
\end{align*}
```
Where $f_0(t)$ indicates the baseline response probability at time $t$. $f_0(t) \sim Weibull(\lambda = 15, k = 1.3)$
...
-   The `TRUE` GROOT for treatment group ($GROOT^{(1)}$), control group ($GROOT^{(0)}$), and GROOT difference ($GD_{true}$) were:
    ...
    -   Large confounding scenario: $GROOT^{(1)} = 0.632$, $GROOT^{(0)} = 0.503$, $GD_{true} = 0.129$
...
-   The `unadjusted GROOT` for treatment group $(GROOT_{unadj} | A = 1)$, control group $(GROOT_{unadj} | A = 0)$, and GROOT difference ($GD_{unadj}$) were slightlty different from the true values in the small congfounding, but increases substaintially in the medium and large confounding scenarios. To be specific 
    ...
    -   Large confounding scenario: $(GROOT_{unadj} | A = 1) = 0.751$, $(GROOT_{unadj} | A = 0) = 0.341$, $GD_{unadj} = 0.410$
...
-   The `absolute biases`, which were measured by the absolute difference between the estimated values and the true values, were high for unadjusted estimate, with the absolute bias ranges from 0.016 in the small confounding scenario, to 0.150 in the medium confounding, and 0.280 in the large confounding scenario. However, the biases were small in adjusted estimates, with the biases range from 0.000 to 0.013 varying for different adjustment methods in all scenarios. The small variations between different methods might be conftributed to the model fit using different smooth parameters. In the scenario 4 (i.e., large confounding with model misspecification), if one of two models (treatment and outcome models) were misspecified, the adjusted estimated (i.e., DB-A and DB-Y) remained unbiased. However, if both models were misspecified, the estimate was bias (DB-AY).
